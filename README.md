...
The latest three question-and-answer exchanges are kept only in Streamlit session state. On later turns, complete topic-specific questions go directly to retrieval unchanged; only context-dependent messages such as “When is it?” or “What should we bring?” are rewritten using recent conversation history. Additive follow-ups containing words such as “other,” “else,” or “more” deterministically inherit missing topic, status, and relative-time terms from the previous question. The guard removes newly invented dates, times, and IDs, and falls back to the current question if a model rewrite drops one of its protected terms. The quick-question buttons remain available after each answer, and a clicked question is queued in session state so it survives Streamlit's rerun without clearing earlier messages. Starting a new conversation or ending the Streamlit session clears this memory. Rewritten follow-up turns therefore make one additional local model call and may be slightly slower than standalone questions.

The Streamlit app opts into a plain-language answer style: answers begin directly, use practical bullets or day-by-day sections when useful, and deterministically render anonymous role IDs as readable descriptions without inventing names. Pending-RSVP questions use an additional evidence gate: only invitation sections explicitly marked `RSVP: pending` are displayed, so unrelated open action items cannot be presented as invitations. Volunteer questions prefer retrieved volunteer-domain documents, and “this week” questions exclude explicitly dated evidence and answer items outside the Monday-through-Sunday week containing the configured reference date. Volunteer-week questions then use a deterministic cited list of scheduled or due commitments, allowing additive follow-ups to compare against the previous answer without relying on the small generation model. If the model returns a cited Markdown section heading such as `Open commitments [S4]`, the UI replaces it with the concrete source bullets beneath that heading rather than presenting the heading as an action. Answers with no resolved citation, including conversational filler, are replaced with the canonical safe refusal. These presentation safeguards are UI-only; evaluation runners retain the original concise prompt so recorded phase comparisons remain reproducible. Inline `[S1]` labels continue to connect factual statements to the resolved documents in the Sources expander.
