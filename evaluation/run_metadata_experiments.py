...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the controlled Phase 7 filtered versus unfiltered dense comparison."
    )
    parser.add_argument(
        "--configs",
        type=Path,
        default=PROJECT_ROOT / "config" / "metadata_experiments",
        help="Directory containing the two metadata_*.yaml experiment files.",
    )
    parser.add_argument(
        "--questions",
        type=Path,
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=PROJECT_ROOT / "evaluation" / "results" / "phase7_metadata",
    )
...
def experiment_config(
    ...
    return {
        ...
        "metadata_filter_enabled": experiment.metadata_filter_enabled,
        "metadata_analyzer": experiment.metadata_analyzer,
        "metadata_facet_version": ANALYZER_VERSION,
        "fallback_to_unfiltered": experiment.fallback_to_unfiltered,
        "semantic_query_preserved": True,
...
def main() -> None:
    ...
    if (base_settings.chunk_size, base_settings.chunk_overlap) != (
        ...
        raise ValueError(
            "Phase 7 must use the selected 500-token chunks with 75-token overlap. "
            "Set RAG_CHUNK_SIZE=500 and RAG_CHUNK_OVERLAP=75."
        )
    if base_settings.top_k != SELECTED_TOP_K:
        raise ValueError("Phase 7 must keep the selected dense retrieval output at Top-5.")
    if base_settings.embedding_model.removesuffix(":latest") != SELECTED_EMBEDDING_MODEL:
        raise ValueError("Phase 7 must keep the selected embeddinggemma embedding model fixed.")

    dataset = load_evaluation_dataset(args.questions)
    experiments = load_metadata_experiments(args.configs)
    ...
    if corpus["document_count"] != 20:
        raise ValueError(
            f"Phase 7 requires the controlled 20-document corpus; found {corpus['document_count']}."
        )
    ...
    vector_store, indexing = rebuild_phase7_metadata_namespace(
        resources,
        namespace,
        enriched_chunks,
    )
    indexing["source_document_count"] = corpus["document_count"]
    indexing["chunk_count"] = len(enriched_chunks)

    output_root = args.output_root.resolve()
    analyzer = MetadataQueryAnalyzer(settings.reference_date)
    completed_results: list[dict[str, Any]] = []
    comparison_rows: list[dict[str, Any]] = []
    for experiment in experiments:
        print(
            f"[{experiment.experiment_id}] Dense Top-{settings.top_k}, "
            f"metadata filters={'enabled' if experiment.metadata_filter_enabled else 'disabled'}..."
        )
    ...
    unfiltered_results, filtered_results = completed_results
    best_recall = max(row["recall_at_5"] for row in comparison_rows)
    comparison = {
        "schema_version": "1.0",
        "phase": 7,
        "experiment_suite": "controlled_metadata_filtering_comparison",
        "completed_at": utc_now(),
        "varied_parameters": ["metadata_filter_enabled", "metadata_analyzer"],
        ...
        "metadata_impact": build_metadata_impact(unfiltered_results, filtered_results),
